...
def main(config: DictConfig):
    ...
    if config.pretrained is not None:
        m = model.model if (config.model._target_.endswith('PARSeq') or 
                           config.model._target_.endswith('PARSeqWithTPS') or
                           config.model._target_.endswith('PARSeqFineTuneDecoder') or
                           config.model._target_.endswith('PARSeqFullFineTune') or
                           config.model._target_.endswith('PARSeqWithAdapters') or
                           config.model._target_.endswith('PARSeqDecoderWithAdapters')) else model
        # Use strict=False for adapter models since adapters won't be in pretrained weights
        strict = not (config.model._target_.endswith('PARSeqWithAdapters') or 
                     config.model._target_.endswith('PARSeqDecoderWithAdapters'))
        if Path(config.pretrained).exists():
            weights = torch.load(config.pretrained, map_location='cpu')
        else:
            weights = get_pretrained_weights(config.pretrained)
        m.load_state_dict(weights, strict=strict)
